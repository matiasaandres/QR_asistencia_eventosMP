...
import { ArrowLeft, KeyRound, LogIn, Mail, QrCode, Send, ShieldCheck, TicketCheck, User, Users } from 'lucide-react';
...
export default function LoginScreen({ portal = 'school', students = [], onGuardianQr }) {
  ...
  const courses = Array.from(new Set(
    students
      .filter((student) => student.status !== 'RETIRADO' && student.course)
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (mode === 'guardian') {
        const found = findStudentForGuardian(students, rut, course);
        if (!found) {
          throw new Error('No encontramos un estudiante activo con ese RUT y curso. Revisa los datos e intenta nuevamente.');
        }
        setGuardianResult(found);
        if (onGuardianQr) onGuardianQr(found);
        return;
      }
      if (mode === 'recover') {
  ...
  const subtitle = mode === 'invite'
    ...
    : mode === 'recover'
      ...
      : mode === 'guardian'
        ? 'Ingresa el RUT y curso del estudiante para obtener su credencial.'
      ...
      <div className="w-full max-w-md rounded-3xl border border-white/20 bg-white shadow-2xl shadow-black/30 overflow-hidden">
        <div className="bg-gradient-to-b from-white to-sky-50 px-7 pb-5 pt-7 text-center">
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-sky-600 text-white shadow-lg shadow-sky-600/25">
            {mode === 'guardian' ? <QrCode className="h-10 w-10" /> : <ShieldCheck className="h-10 w-10" />}
          </div>
          <h1 className="mt-4 text-2xl font-black text-slate-950">{title}</h1>
          <p className="mt-1 text-sm text-slate-500">{subtitle}</p>
        </div>

        {mode === 'guardian' && guardianResult ? (
          <div className="space-y-4 px-7 pb-7 pt-2 text-center">
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
              <p className="text-xs font-extrabold uppercase tracking-wider text-emerald-800">{guardianResult.course}</p>
              <h2 className="mt-1 text-lg font-black text-slate-900">{guardianResult.name}</h2>
              <p className="mt-1 font-mono text-xs text-slate-500">Código: {guardianResult.id}</p>
            </div>
            <div className="flex justify-center p-2 bg-white rounded-2xl border border-slate-100 shadow-inner">
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(guardianResult.id)}`}
                alt={`Código QR de ${guardianResult.name}`}
                className="h-44 w-44 rounded-xl"
              />
            </div>
            <p className="text-xs text-slate-500">Muestra este código QR en portería al ingresar al evento.</p>
            <button
              type="button"
              onClick={() => { setGuardianResult(null); setRut(''); }}
              className="flex w-full items-center justify-center gap-1 text-xs font-bold text-sky-700 hover:underline"
            >
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-1 text-xs font-bold text-slate-500 hover:underline"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4 px-7 pb-7 pt-2" noValidate>
                ...
                <label className="block text-sm font-bold text-slate-700">
                  ...
                  <span className="relative mt-1.5 block">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-11 pr-3 outline-none focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-100"
                  ...
                  <span className="relative mt-1.5 block">
                    {courses.length > 0 ? (
                      <select
                        ...
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-slate-900 outline-none focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-100"
                      >
            ...
            <button disabled={isSaving} className="flex w-full items-center justify-center gap-2 rounded-xl bg-sky-600 py-3 text-sm font-extrabold text-white shadow-lg shadow-sky-600/25 hover:bg-sky-500 disabled:opacity-60">
              {mode === 'guardian' ? <QrCode className="h-5 w-5" /> : mode === 'recover' ? <Send className="h-5 w-5" /> : mode === 'invite' ? <TicketCheck className="h-5 w-5" /> : <LogIn className="h-5 w-5" />}
              {isSaving ? 'Procesando…' : mode === 'guardian' ? 'Obtener mi código QR' : mode === 'recover' ? 'Enviar enlace de recuperación' : mode === 'invite' ? 'Aceptar e ingresar a la escuela' : 'Ingresar a la aplicación'}
            </button>
            {mode === 'login' ? <div className="space-y-2 text-center">
              ...
              {portal === 'school' && <button type="button" onClick={() => changeMode('guardian')} className="block w-full text-xs font-bold text-emerald-700 hover:underline">Recuperar código QR (Apoderados)</button>}
