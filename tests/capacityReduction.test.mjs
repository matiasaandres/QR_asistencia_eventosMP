...
test('la actualización de cupos escribe solo capacidad, sin contadores ni bitácora', () => {
  ...
  const update = source.split('export async function saveStudentCapacities')[1].split('export async function migrateLegacyFamilies')[0];
  assert.match(update, /eventFamilyDoc\(db, organizationId, eventId, id\)/);
  assert.match(update, /eventStudentDoc\(db, organizationId, eventId, id\)/);
  assert.match(update, /ownerCapacities/);
  assert.doesNotMatch(update, /enteredCount|extraGuest|lastEntryAt/);
});
